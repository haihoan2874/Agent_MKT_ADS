...
const updateStatus = (dir: string, step: number, total: number, message: string) => {
  try {
...
export async function runPipeline(scriptPath: string): Promise<void> {
  ...
  // STEP 1
  const s1Msg = `Load env + validate script.json (TTS provider: ${cfg.ttsProvider})`;
  log.step(1, TOTAL_STEPS, s1Msg);
  updateStatus(outputDir, 1, TOTAL_STEPS, s1Msg);
  ...
  // STEP 2
  const s2Msg = "Write script.txt for CapCut";
  log.step(2, TOTAL_STEPS, s2Msg);
  updateStatus(outputDir, 2, TOTAL_STEPS, s2Msg);
  ...
  const s3Msg = "Fetch og:image (parallel) + Step 4 TTS";
  log.step(3, TOTAL_STEPS, s3Msg);
  updateStatus(outputDir, 3, TOTAL_STEPS, s3Msg);
  const imgPath = join(outputDir, "images", "bg.jpg");
  const imgPromise = fetchImage(script.metadata.source.image, imgPath);

  // STEP 4
  const ttsClient = createTtsClient(cfg, script.voice.voiceId);
  ...
  // STEP 5
  const s5Msg = "Concat voice scenes + mix SFX layer";
  log.step(5, TOTAL_STEPS, s5Msg);
  updateStatus(outputDir, 5, TOTAL_STEPS, s5Msg);
  ...
  const s6Msg = "Compose HTML + project files";
  log.step(6, TOTAL_STEPS, s6Msg);
  updateStatus(outputDir, 6, TOTAL_STEPS, s6Msg);
  ...
  // STEP 7
  const s7Msg = "Render with hyperframes";
  log.step(7, TOTAL_STEPS, s7Msg);
  updateStatus(outputDir, 7, TOTAL_STEPS, s7Msg);
  const videoPath = join(outputDir, "video.mp4");
  await renderWithHyperframes({ compositionDir: outputDir, outputPath: videoPath });

  // STEP 8
  const s8Msg = "Done";
  log.step(8, TOTAL_STEPS, s8Msg);
  updateStatus(outputDir, 8, TOTAL_STEPS, s8Msg);
  console.log("\n=== Result ===");
