...
## 🏗️ S-LIFE TechHub Design System (PROJECT-SPECIFIC)

> **MANDATORY:** All development in this repository must use the S-LIFE Techwear system.

### 🎨 Design Principles
1. **No Excessive Italics**: Do not use `italic` for common UI text (buttons, menus, titles). Keep it straight and bold for a professional "Hardcore Tech" look.
2. **Uppercase Focus**: Most interactive elements should use `uppercase` and `font-black`.
3. **UI Kit Priority**: NEVER build custom UI elements if they exist in the UI Kit.

### 🧱 Mandatory UI Kit Usage
- `src/components/ui/Button.jsx`: Use for ALL buttons.
- `src/components/ui/Input.jsx`: Use for ALL text inputs/search bars.
- `src/components/ui/Modal.jsx`: Use for ALL popups.
- `src/components/ui/ConfirmModal.jsx`: Use for ALL sensitive actions (Logout, Delete, etc.).
- `src/components/ui/ProductCard.jsx`: Use for product displays.
